...
public class AdminUnitRepositoryBuilder {
    ...
    Storage storage;
    // TODO: bucketName
    @Value("${blobstore.gcs.kakka.bucket.name:kakka-dev}")
    ...
    @Value("${bahamut.workdir:/tmp/bahamut/adminUnits}")
    private String localWorkingDirectory;

    @PostConstruct
    public void init() {
        repository.setStorage(storage);
        repository.setBucketName(containerName);
    }
        ...
        public void buildNewCache() throws IOException {
            ...
            for (BlobStoreFiles.File blob : blobs.getFiles()) {
                if (blob.getName().endsWith(".zip")) {
                    ZipUtilities.unzipFile(repository.getBlob(blob.getName()), localWorkingDirectory);
                } else if (blob.getName().endsWith(".xml")) { // TODO: Trenger vi then sjekken her?
                    try {
