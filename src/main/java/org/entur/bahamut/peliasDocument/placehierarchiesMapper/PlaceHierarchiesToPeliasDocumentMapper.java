...
public abstract class PlaceHierarchiesToPeliasDocumentMapper<T extends Place_VersionStructure> {
    ...
    private PeliasDocument toPeliasDocument(PlaceHierarchy<T> placeHierarchy, MultilingualString name, int idx) {
        ...
        var document = new PeliasDocument(getLayer(place), place.getId() + idSuffix, findSource(placeHierarchy));
        if (name != null) {
        ...
        if (place.getPolygon() != null) {
            ...
            document.setShape(PlaceHierarchiesMapperUtilities.toPolygon(place.getPolygon().getExterior().getAbstractRing().getValue()));
        }
    ...
    protected String findSource(PlaceHierarchy<T> hierarchy) {
    ...
    protected boolean isValid(T place) {
        ...
        return PlaceHierarchiesMapperUtilities.isValid(place);
    }
    ...
    private void addIdToStreetNameToAvoidFalseDuplicates(T place, PeliasDocument document) {
        ...
        document.addressParts().setStreet("NOT_AN_ADDRESS-" + place.getId());
    }
